...
// Ensure last_log_ts is present at startup (prevents immediate watcher restart)
try {
  writeLastLogTs(Date.now());
  console.log(isoNow(), 'wrote initial last_log_ts on startup');
} catch (e) {
  console.error(isoNow(), 'failed to write initial last_log_ts', e && e.message);
}
...
app.listen(PORT, () => {
  ...
  try{
    // allow disabling watcher via env (useful when deploying to platforms that already supervise processes)
    if (process.env.WATCHER_ENABLED === 'false') {
      log('WATCHER_ENABLED=false — not spawning watcher');
    } else {
